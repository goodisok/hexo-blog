...
![四种合成数据生成方法对比：2D 合成快但不真实，Blender+HDRI 是最佳平衡点，UE5 对纯天空场景过于重量级，3DGS 需要先有真实数据](/images/blender-hdri-drone/method_comparison.png)
...
![系统架构：3D 模型 + HDRI 天空 + 配置参数 → Blender Cycles 渲染（IBL 光照 + 域随机化 + BBox 投影）→ JPEG 图像 + YOLO 标注 + dataset.yaml](/images/blender-hdri-drone/pipeline_architecture.png)
...
![五种观测构型示意图：尾追（35%）、侧方接近（25%）、迎头（15%）、上方后方（15%）、下方近距（10%），按加权概率随机采样](/images/blender-hdri-drone/engagement_profiles.png)
