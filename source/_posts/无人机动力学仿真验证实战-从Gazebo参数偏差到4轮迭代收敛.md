...
| 速度段 | k_f 辨识误差 | 诊断含义 |
|--------|------------|---------|
| 10-25 m/s | 3-4% | 模型结构在此区间充分准确 |
| 0-5 m/s | 15-21% | 低信噪比 + 低速阻力扰动 |
| 40-50 m/s | 9-15% | **模型结构开始不足** |

如果模型结构完美，k_f 的辨识值在所有速度段应当一致。k_f 随速度单调漂移说明：在高速段，未建模的气动效应（入流效应、非线性阻力）被"吸收"进了 k_f 估计中，导致等效 k_f 偏大。
...
1. 逐步提高飞行速度（15 → 25 → 35 → 50 m/s）
2. 每个速度段独立辨识 k_f，观察是否随速度漂移
3. 如果漂移 > 10%：阻力模型结构不足 → 改为非线性阻力
4. Setpoint replay 验证各速度段的匹配度
