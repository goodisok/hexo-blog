---
title: 四旋翼 Sim-to-Real 动力学验证全流程：Gazebo+PX4 从参数辨识到 97 m/s 高速截击机建模
...
>
> 本文用一个**纯仿真闭环实验**完整走通了 Sim-to-Real 动力学验证流程。实验方法是**设定点回放（Setpoint Replay）**——从"真值"飞机的 ULG 日志中提取 trajectory_setpoint，回放给"待校准"的仿真模型，比较两者在相同目标下的动力学响应差异。4 轮迭代参数修正后，垂直速度 RMSE 下降 48%，俯仰角 RMSE 下降 45%，偏航角速度 RMSE 下降 42%。进一步扩展到 Gobi 类截击机场景（97 m/s），发现 Gazebo 线性阻力模型在 50+ m/s 完全失效，并开发了 `QuadraticDrag` 自定义插件实现物理正确的 v² 阻力。最终给出了从方法验证到外场截击机建模的完整路线图。
>
