...
> AirSim/Colosseum 的四旋翼仿真不是黑盒——但它也**不是简单地读一个 JSON 就能完全自定义的**。本文基于 Colosseum 仓库 [1] 的真实 C++ 源码逐文件解析，**精确指出哪些参数硬编码在 C++ 中、哪些可以通过 `settings.json` 配置、哪些需要修改源码才能改变**，手把手讲解如何为自己的无人机建立准确的仿真模型。
...
## 二、源码架构：基于真实仓库目录的准确描述

### 2.1 核心目录结构

以下目录结构基于 Colosseum 仓库 [1] 的 `main` 分支实际文件列表：
...
│   └── vehicles/multirotor/
│       ├── MultiRotorPhysicsBody.hpp     # ★ 多旋翼刚体（不是 MultiRotor.hpp！）
...
**注意**：仓库中**不存在** `MultiRotor.hpp`。多旋翼物理体的实现类是 **`MultiRotorPhysicsBody`**，定义在 `MultiRotorPhysicsBody.hpp` 中。
...
## 三、RotorParams：螺旋桨参数的真实定义

### 3.1 源码原文（`RotorParams.hpp`）

这是 AirSim 中**所有螺旋桨/电机参数**的完整定义。以下代码直接来自 Colosseum 源码 [1]：
...
> **重要纠正**：之前版本的文章错误地声称这些参数可以通过 `settings.json` 中的 `RotorParams` 字段配置。**实际上 `settings.json` 的 `VehicleSetting` 解析代码中完全不存在对这些物理参数的读取逻辑**。要修改这些值，必须修改 C++ 源码并重新编译。

---

## 四、RotorActuator：电机执行器的真实实现
...
源码中 `setOutput()` 的实现（**这是实际代码，不是伪代码**）：
...
这意味着**推力与控制信号成正比**（不是与转速平方成正比！），而转速的计算是 $\omega = \sqrt{u_f} \times \omega_{\max}$，保证了 $T = k_T \omega^2$ 的物理一致性：
...
AirSim **不使用** `tau_up`/`tau_down` 这样的分段时间常数。实际使用的是 `FirstOrderFilter`，统一的一阶低通滤波器，时间常数为 `control_signal_filter_tc`（默认 0.005 s）：
...
## 五、MultiRotorPhysicsBody：多旋翼物理体的真实结构
...
**这是 AirSim 阻力模型的真实实现**——不是简单的线性阻力 $F_d = -k_d v$，而是基于物理的**六面体面阻力模型**：
...
## 六、FastPhysicsEngine：物理引擎的真实积分方法
...
### 6.5 Velocity Verlet 积分（不是简单欧拉！）

**重要纠正**：AirSim 使用的是 **Velocity Verlet 积分**（也叫 Störmer-Verlet），不是简单的一阶欧拉积分：
...
**注意**：这里使用的是**精确的轴角表示**（`AngleAxisr`），而不是之前文章中描述的小角度近似 $\Delta q \approx [1, \frac{\omega_x \Delta t}{2}, \frac{\omega_y \Delta t}{2}, \frac{\omega_z \Delta t}{2}]$。轴角转四元数的公式为：
...
基于源码分析，一个**真正有效**的 SimpleFlight 多旋翼配置如下：
...
> **注意**：你在网上找到的很多教程中 `settings.json` 包含 `"Mass"`, `"Inertia"`, `"RotorParams"` 等字段的配置**是错误的**——这些字段不会被 Colosseum 的 `AirSimSettings.hpp` 解析。如果你写了这些字段，程序不会报错（JSON 解析时会忽略未知字段），但**参数不会生效**。
