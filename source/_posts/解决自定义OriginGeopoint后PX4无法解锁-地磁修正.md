...
在使用 AirSim/Colosseum 时，若在 `settings.json` 里配置了自定义的 **`OriginGeopoint`**（例如配合 Cesium 指定真实地理起点），有时会出现 **PX4 无法解锁** 的情况，并伴随 `Preflight Fail: heading estimate not stable`、`Yaw estimate error`、`GPS fix too low` 等告警。社区 [AirSim #5001](https://github.com/Microsoft/AirSim/issues/5001) 中有人遇到相同现象，并指出根本原因是 **磁强计读数与自定义起点位置不一致**。本文整理该问题的原因与解决方案：通过基于 **WMM（世界地磁模型）** 的库重写 `EarthUtils.hpp` 中的 **`getMagField`**（以及 `getMagDeclination`），使仿真中的磁强计输出与当前 `OriginGeopoint` 对应的真实地磁一致，从而消除航向估计错误、使 PX4 可正常解锁。
...
PX4 的 EKF（扩展卡尔曼滤波）使用 **磁强计（罗盘）** 参与航向估计。仿真里磁强计的数据由 AirLib 的 **`EarthUtils::getMagField`** 根据当前**地理点**（经纬度、高度）计算地磁矢量（北/东/地分量），再结合机体姿态得到“传感器读数”。
...
因此，**自定义 `OriginGeopoint` 后，必须按“该起点对应的经纬度、高度”计算地磁，磁强计仿真才正确。**
...
- **`getMagField`** 若未按当前地理点使用 WMM 计算，则任意自定义起点下的磁强计读数都会偏离真实值。

一旦用户把 `OriginGeopoint` 设到阿姆斯特丹、北京等任意地点，仿真仍按“默认原点”或粗表给磁强计，就会导致上述 Preflight 失败。

### 2.3 关于 GPS fix too low

**GPS fix too low** 通常与“GPS 位置/高度是否与 Unreal 场景、Cesium 原点一致”有关，属于**位置对齐**问题，与地磁无直接关系。但 **heading estimate not stable** 的解决（见下文）能消除因磁强计错误导致的解锁障碍；若仍有 GPS 告警，需单独检查仿真中 GPS 与 `OriginGeopoint`、场景原点的一致性。
...
这样，无论 `OriginGeopoint` 设在哪里，磁强计仿真都使用**该地点、该年代**的真实地磁，PX4 的航向估计与 Preflight 检查才能通过。
...
这样，仿真中的磁强计与 PX4 的期望一致，**heading estimate not stable**、**Yaw estimate error** 即可消除。
...
- **原因**：仿真中磁强计使用的 **`getMagField`**（及磁偏角）未按该起点经纬度、高度计算，与真实地磁不一致，导致 EKF 航向估计异常。
- **解决**：在 **`EarthUtils.hpp`** 中，用基于 WMM 的库（如 [nhz2/XYZgeomag](https://github.com/nhz2/XYZgeomag) 或头文件版 **geomag.hpp**）重写 **`getMagField`** 与 **`getMagDeclination`**，使磁强计输出与当前 `OriginGeopoint` 对应的真实地磁一致，即可消除上述 Preflight 失败，正常解锁。
